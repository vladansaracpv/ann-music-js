...
// Only sharp notes
Theory.SHARPS_ONLY = Theory.ALL_NOTES.filter(x => x.includes('#'));
// Only flat notes
Theory.FLATS_ONLY = Theory.ALL_NOTES.filter(x => x.includes('b'));
// Only natural notes (without sharps or flats)
Theory.NATURALS_ONLY = Theory.ALL_NOTES.filter(x => x.length === 1);
// Natural + Sharps
Theory.SHARPS = Theory.ALL_NOTES.filter(x => Theory.FLATS_ONLY.indexOf(x) === -1);
// Natural + Flats
Theory.FLATS = Theory.ALL_NOTES.filter(x => Theory.SHARPS_ONLY.indexOf(x) === -1);
